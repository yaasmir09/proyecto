...
\restrict dOMHuOdmqX4ecwE5XifEA9SeIqvKRKSoiobVJFAGrfI1fhagXhZ0cGBxe40FHbW
...
CREATE TABLE public.clientes (
    ...
    CONSTRAINT clientes_estado_check CHECK (((estado)::text = ANY (ARRAY[('activo'::character varying)::text, ('inactivo'::character varying)::text, ('riesgo'::character varying)::text])))
);
...
CREATE TABLE public.cuotas (
    ...
    CONSTRAINT cuotas_estado_check CHECK (((estado)::text = ANY (ARRAY[('pendiente'::character varying)::text, ('pagada'::character varying)::text, ('vencida'::character varying)::text, ('parcial'::character varying)::text])))
);
...
CREATE TABLE public.facturas (
    ...
    creado_en timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT facturas_estado_check CHECK (((estado)::text = ANY (ARRAY[('activa'::character varying)::text, ('vencida'::character varying)::text, ('pagada'::character varying)::text, ('revision'::character varying)::text])))
);
...
CREATE TABLE public.prestamos (
    ...
    creado_en timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    actualizado_en timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT prestamos_estado_check CHECK (((estado)::text = ANY (ARRAY[('activo'::character varying)::text, ('pagado'::character varying)::text, ('vencido'::character varying)::text, ('cancelado'::character varying)::text])))
);
...
6	Keny Gonzalez	kenygonzalez@gmail.com	610-090103-1003J	89544567	UNAN	activo	2026-08-19 21:16:58.101395
\.
...
\unrestrict dOMHuOdmqX4ecwE5XifEA9SeIqvKRKSoiobVJFAGrfI1fhagXhZ0cGBxe40FHbW
